    ...
    module HTML
      ...
      def r_rename_asset
        ...
        when 'style'
          ...
          return expand_with.gsub(/url\(('|")(.*?)\1\)/) do
            if $2[0..6] == 'http://'
              ...
              quote   = $1
              new_src = helper.send(:template_url_for_asset, :current_folder=>@options[:current_folder], :src => $2)
        ...
        if src && src[0..0] != '/' && src[0..6] != 'http://'
          @params[key] = helper.send(:template_url_for_asset, :src => src, :current_folder => @options[:current_folder], :type => type)
        end
